...
void handle_server_msg()
{
  ...
  if ( bytes_read != -1 )
  {
    /* THIS CODE SUCKS PLEASE REPLACE ME */
    past_data_read.append( buffer );
    cout << past_data_read << endl;
    ...
    int good_first_char = 0;
    while ( ! good_first_char ) // or no chars
    {
      switch( past_data_read.front() )
      {
        case EXIT:
          good_first_char = 1;
          break;
        ...
        default:
          if ( past_data_read.length() != 0 )
          {
            past_data_read = past_data_read.substr(1);
          }
          else
          {
            good_first_char = 1;
    ...
    cout << past_data_read.length() << endl;
    /* END OF SUCKY CODE */
